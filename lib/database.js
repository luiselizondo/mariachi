...
Connection.prototype.getServersWithStatus = function(status, callback) {
	var query = "SELECT * FROM servers WHERE status = ?";
	connection.query(query, status, function(err, rows, fields) {
		callback(err, rows);
	});	
}
...
Connection.prototype.getTask = function(id, callback) {
	...
	function joinProjects(id, callback) {
	...
	connection.query(query, id, function (err, rows, fields) {
		...
		if(rows) {
			...
			result.parameters = JSON.parse(rows[0].parameters);
			...
			if(result.type == "project") {
				joinProjects(result.task, function (err, r) {
					result.recepieName = null;
					result.templateName = null;
...
Connection.prototype.getTasks = function(callback) {
	...
	// main query
	var query = "SELECT t.*, s.name as serverName, u.name as userName FROM tasks t LEFT JOIN servers s ON t.server = s.id LEFT JOIN users u ON t.user = u.id ORDER BY id DESC limit 10";
	connection.query(query, function(err, rows, fields) {
		if(err) {
			console.log(err);
			callback(err, rows);
...
Connection.prototype.getProject = function(id, callback) {
	console.log("Getting project with id: %s", id);
	var self = this;
	var query = "SELECT * FROM projects WHERE id = ?";
	connection.query(query, id, function(err, rows, fields) {
		if(err) {
		...
		if(rows) {
			...
			if(!_.isUndefined(result.data.webserver)) {
				self.getServer(result.data.webserver, function(err, webserver) {
					if(err) result.webserver = null;
					if(webserver) result.webserver = webserver;

					if(!_.isUndefined(result.data.databaseServer)) {
						self.getServer(result.data.databaseServer, function(err, databaseServer) {
							if(err) result.databaseServer = null;
							if(databaseServer) result.databaseServer = databaseServer;
			...
			else if(!_.isUndefined(result.data.databaseServer)) {
				self.getServer(result.data.databaseServer, function(err, databaseServer) {
					if(err) result.databaseServer = null;
					if(databaseServer) result.databaseServer = databaseServer;
