...
function taskStart(socket, data) {
	socket.emit("tasks:start", {stderr: data.stderr, stdout: data.stdout, status: data.status});
}
...
function taskStream(socket, data) {
	socket.emit("tasks:stream", {stderr: data.stderr, stdout: data.stdout, status: data.status});
}
